...
class Tensorial
{
 ...
 public:
  ...
  template <typename point_t,
            ...
            point_t fun(const point_t&, const return_t),
            typename smallVec_t = point_t>
  static return_t integrate_bdr_phiphi_cutwind(const unsigned int i,
                                               const unsigned int j,
                                               const unsigned int bdr,
                                               geom_t& geom,
                                               return_t f_param = 0.)
  {
    ...
    for (unsigned int q = 0; q < std::pow(quad_weights.size(), geom_t::hyEdge_dim() - 1); ++q)
    {
      ...
      winding = scalar_product(normal, fun(geom.map_ref_to_phys(quad_pt), f_param));
      if (winding > 0)
        integral += geom.face_area(bdr) * quad_weight * phi_i * phi_j;
    }
  ...
  template <typename point_t,
            ...
            point_t fun(const point_t&, const return_t),
            typename smallVec_t = point_t>
  static return_t integrate_bdr_phipsi_cutdownwind(const unsigned int i,
                                                   const unsigned int j,
                                                   const unsigned int bdr,
                                                   geom_t& geom,
                                                   return_t f_param = 0.)
  {
    ...
    for (unsigned int q = 0; q < std::pow(quad_weights.size(), geom_t::hyEdge_dim() - 1); ++q)
    {
      ...
      if (winding > 0)
        integral += geom.face_area(bdr) * quad_weight * phi_i * psi_j;
    }
  ...
  template <typename point_t,
            ...
            point_t fun(const point_t&, const return_t),
            typename smallVec_t = point_t>
  static return_t integrate_bdr_psiphi_cutwind(const unsigned int i,
                                               const unsigned int j,
                                               const unsigned int bdr,
                                               geom_t& geom,
                                               return_t f_param = 0.)
  {
    ...
    for (unsigned int q = 0; q < std::pow(quad_weights.size(), geom_t::hyEdge_dim() - 1); ++q)
    {
      ...
      winding = scalar_product(normal, fun(geom.map_ref_to_phys(quad_pt), f_param));
      if (winding > 0)
        integral += geom.face_area(bdr) * quad_weight * psi_i * phi_j;
    }
    return integral;
  }
  ...
  template <typename point_t,
            ...
            point_t fun(const point_t&, const return_t),
            typename smallVec_t = point_t>
  static return_t integrate_bdr_psipsi_cutdownwind(const unsigned int i,
                                                   const unsigned int j,
                                                   const unsigned int bdr,
                                                   geom_t& geom,
                                                   return_t f_param = 0.)
  {
    ...
    for (unsigned int q = 0; q < std::pow(quad_weights.size(), geom_t::hyEdge_dim() - 1); ++q)
    {
      ...
      winding = scalar_product(normal, fun(geom.map_ref_to_phys(quad_pt), f_param));
      if (winding > 0)
        integral += geom.face_area(bdr) * quad_weight * psi_i * psi_j;
    }
    return integral;
  }
  /*!***********************************************************************************************
                                                         ...
                                                         return_t f_param = 0.)
  {
    ...
    for (unsigned int q = 0; q < std::pow(quad_weights.size(), geom_t::hyEdge_dim() - 1); ++q)
    {
      ...
      winding = scalar_product(normal, fun(geom.map_ref_to_phys(quad_pt), f_param));
      if (winding < 0)
        integral += geom.face_area(bdr) * quad_weight * psi_i *
                    func(geom.map_ref_to_phys(quad_pt), f_param) * winding;
    }
    return integral;
  }
